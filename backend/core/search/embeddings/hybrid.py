...
def embed_documents_hybrid(texts: List[str], force_method: Optional[str] = None) -> List[List[float]]:
        ...
        try:
            ...
            if embedder is not None:
                ...
                if embeddings and len(embeddings) == len(texts):
                    # 차원 통일 (sentence-transformers 384 -> OpenAI 1536)
                    embeddings = normalize_embeddings_dimension(embeddings, TARGET_DIMENSION)
                    
                    elapsed = time.time() - start_time
                    logger.info(f"{device_name.get(method, 'sentence-transformers')} 임베딩 완료: {len(embeddings)}개, 차원: {len(embeddings[0]) if embeddings else 0}, {elapsed:.2f}초")
                    return embeddings
                else:
    ...
    if method == "openai" or GPU_FALLBACK_TO_OPENAI:
        ...
        try:
            ...
            if embeddings and len(embeddings) == len(texts):
                # OpenAI는 이미 1536차원이므로 normalize 불필요하지만 안전을 위해 호출
                embeddings = normalize_embeddings_dimension(embeddings, TARGET_DIMENSION)
                
                elapsed = time.time() - start_time
                logger.info(f"OpenAI 임베딩 완료: {len(embeddings)}개, 차원: {len(embeddings[0]) if embeddings else 0}, {elapsed:.2f}초")
                return embeddings
            else:
...
def log_embedding_status():
    ...
    logger.info(f"벡터 차원 통일: {TARGET_DIMENSION}차원")
    logger.info("========================")
