...
class TenantContext:
    ...
    def __init__(self, company_id: int, platform: str = "freshdesk"):
    ...
    def config_manager(self) -> TenantConfigManager:
        ...
        if self._config_manager is None:
            self._config_manager = get_tenant_config_manager(str(self.company_id), self.platform)
    ...
    def __repr__(self):
        return f"TenantContext(company_id={self.company_id}, platform='{self.platform}')"
...
def set_current_tenant(company_id: int, platform: str = "freshdesk") -> TenantContext:
    """현재 요청의 테넌트 컨텍스트 설정"""
    context = TenantContext(company_id, platform)
...
@contextmanager
def tenant_context(company_id: int, platform: str = "freshdesk"):
    """테넌트 컨텍스트 관리 컨텍스트 매니저
    
    Usage:
        with tenant_context(company_id=1, platform="freshdesk"):
            config = get_current_tenant().get_freshdesk_config()
    """
    old_context = get_current_tenant()
    try:
        set_current_tenant(company_id, platform)
...
def with_tenant_context(company_id_param: str = 'company_id', 
                       platform_param: str = 'platform'):
    """함수 매개변수에서 테넌트 정보를 추출하여 컨텍스트를 설정하는 데코레이터
    
    Args:
        company_id_param: company_id가 들어있는 매개변수명
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if company_id is None:
                ...
                if company_id_param in param_names:
                    param_index = param_names.index(company_id_param)
                    if param_index < len(args):
                        company_id = args[param_index]
            
            if company_id is None:
                raise ValueError(f"매개변수 '{company_id_param}'에서 company_id를 찾을 수 없습니다.")
            
            # 테넌트 컨텍스트 설정하고 함수 실행
            with tenant_context(int(company_id), platform):
                return func(*args, **kwargs)
...
def extract_tenant_from_request(request) -> tuple[int, str]:
    ...
    if company_id:
        return int(company_id), platform
    
    # 2. 쿼리 파라미터에서 추출
    company_id = request.query_params.get('company_id') or request.query_params.get('tenant_id')
    platform = request.query_params.get('platform', 'freshdesk')
    
    if company_id:
        return int(company_id), platform
    
    # 3. 서브도메인에서 추출 (예: acme.yourdomain.com -> company_id는 DB에서 조회)
    host = request.headers.get('host', '')
    if '.' in host:
        subdomain = host.split('.')[0]
        # 실제로는 DB에서 subdomain -> company_id 매핑을 조회해야 함
        # 여기서는 예시로 하드코딩
        subdomain_mapping = {
            'wedosoft': 1,
            'acme': 2,
...
async def tenant_middleware(request, call_next):
    ...
    try:
        company_id, platform = extract_tenant_from_request(request)
