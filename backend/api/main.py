    ...
    try:
        ...
        try:
            ...
            async def generate_ticket_summary():
                """티켓 요약 생성 태스크"""
                if settings.FAST_INIT_MODE:
                    logger.info("⚡ FAST_INIT_MODE 활성화: LLM 요약 생략")
                    return f"## 📋 티켓 요약\n**제목**: {ticket_title}\n\n{ticket_body[:200]}"
...
## 📊 분석"""
            ...
            if similar_tickets_temp and include_similar_tickets:
                try:
                    ...
                    if settings.FAST_INIT_MODE:
                        logger.info("⚡ FAST_INIT_MODE 활성화: 유사 티켓 분석 생략")
                        for ticket in similar_tickets_temp:
                            similar_tickets.append({
                                "id": ticket["id"],
                                "title": ticket["title"],
                                "issue": ticket["ticket_text"][:100],
                                "solution": "상세 내용 확인 필요",
