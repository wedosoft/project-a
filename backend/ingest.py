...
def sanitize_metadata(
    ...
    """
    메타데이터 값을 ChromaDB 호환 형식으로 변환합니다.

    ChromaDB는 메타데이터 값으로 문자열, 정수, 소수점, 불리언만 허용합니다.
    ...
    Returns:
        ChromaDB 호환 형식으로 변환된 메타데이터 딕셔너리
    """
    sanitized = {}
    ...
    try:
        ...
        if COLLECTION_NAME not in collection_names:
            ...
            if existing_count == 0 and os.path.exists(DB_PATH):
                ...
                if folder_size_mb > 10:  # 10MB 이상인데 문서가 없으면 불일치로 판단
                    logger.warning(
                        f"데이터베이스 불일치 감지: 폴더 크기 {folder_size_mb:.2f}MB인데 문서 수 0개"
                    )
        ...
        for t in tickets:
            ...
            if doc_text.strip():  # 빈 문서 제외
                ...
                # 메타데이터 크기가 너무 크면 ChromaDB에서 오류가 발생할 수 있으므로
                # 기본 필드만 유지하고 나머지는 제외
...
async def update_status_mappings(collection_name: str = COLLECTION_NAME) -> None:
    ...
    이 함수는 상태 텍스트 값이 변경되었을 때 기존 문서에 새로운 상태 텍스트를 적용합니다.
    상태 코드(숫자)를 기준으로 매핑하므로 원본 정보는 유지됩니다.
    """
    logger.info("상태 매핑 정보 업데이트 및 적용 시작")
    ...
    try:
        ...
        logger.info(f"총 {len(all_ids)}개 문서의 상태 매핑 업데이트 검토 중...")
        ...
        for i, (doc_id, metadata) in enumerate(zip(all_ids, all_metadatas)):
            ...
            # 타입과 상태 코드가 있는 경우에만 처리
            if doc_type and status_code is not None:
                # 문자열로 된 숫자를 정수로 변환
                if isinstance(status_code, str) and status_code.isdigit():
                    status_code = int(status_code)

                # 해당 타입의 상태 매핑 가져오기
                status_map = STATUS_MAPPINGS.get(doc_type, {})

                # 새 상태 텍스트 결정
                new_status = None
                if doc_type == "ticket" and status_code in status_map:
                    new_status = status_map[status_code]
                elif doc_type == "kb" and status_code in status_map:
                    new_status = status_map[status_code]

                # 현재 상태와 다른 경우 업데이트 필요
                current_status = metadata.get("status")
                if new_status and new_status != current_status:
                    logger.info(
                        f"문서 {doc_id}: 상태 업데이트 필요 - {current_status} -> {new_status}"
                    )
        ...
        # 업데이트 실행
        if updates_needed:
            logger.info(f"총 {len(updates_needed)}개 문서의 상태 값 업데이트 중...")
            ...
            for i in range(0, len(updates_needed), batch_size):
                ...
                batch_documents = [item["document"] for item in batch]
                batch_embeddings = [item["embedding"] for item in batch]

                logger.info(
                    f"문서 배치 업데이트 중... ({i+1}~{i+len(batch)}/{len(updates_needed)})"
                )
                collection.upsert(
                    ids=batch_ids,
                    metadatas=batch_metadatas,
            ...
            logger.info(
                f"상태 매핑 업데이트 완료. {len(updates_needed)}개 문서 업데이트됨."
            )
        else:
            logger.info("업데이트가 필요한 문서가 없습니다.")
