...
try:
    ...
    from ..langchain.smart_conversation_filter import SmartConversationFilter
except ImportError:
    from backend.core.retriever import retrieve_top_k_docs
    from backend.core.search_optimizer import VectorSearchOptimizer
    from backend.core.langchain.smart_conversation_filter import SmartConversationFilter
...
class LLMManager:
    ...
    async def generate(self, 
        ...
        """
        # 용도별 모델 설정 적용
        config = self._get_model_config_for_use_case(use_case)
        if config:
    ...
    def _get_model_config_for_use_case(self, use_case: str) -> Optional[Dict[str, Any]]:
        """용도에 따른 모델 설정 반환"""
    ...
    def _filter_meaningful_conversations(self, conversations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        스마트 대화 필터링 시스템을 사용한 의미있는 대화 선택
        
        기존의 5개 제한을 제거하고 스마트 필터링 적용:
    ...
    def _is_meaningful_conversation(self, conv: Dict[str, Any]) -> bool:
        ...
        # 단순 인사말 제외 (짧고 의미 없는 내용)
        if len(body) < 30:
            greeting_patterns = [
                "안녕하세요", "안녕", "hello", "hi", "수고하세요", "수고", 
                "감사", "thank", "확인", "네", "예", "yes", "ok", "okay"
            ]
            if any(pattern.lower() in body_lower for pattern in greeting_patterns):
                return False
        
        # 단순 확인이나 승인 메시지 제외
        simple_confirmations = [
            "확인했습니다", "확인됐습니다", "알겠습니다", "understood", 
            "got it", "received", "noted", "확인완료", "처리완료"
        ]
        if len(body) < 50 and any(pattern.lower() in body_lower for pattern in simple_confirmations):
            return False
        ...
        try:
            ...
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    ...
                    logger.info(f"다국어 키워드 패턴 로드 완료: {list(patterns.keys())} 언어")
                    return patterns
            else:
