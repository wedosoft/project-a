...
## 🏗️ **모듈화된 아키텍처 패턴 (2025년 6월 21일 업데이트)**

### 💡 **LLM 라우터 모듈화 (core/llm/)**

**모듈 구조** (llm_router.py 분해 완료):

```
core/llm/
├── router.py      # LLM 라우팅 메인 로직 (LLMRouter 클래스)
├── clients.py     # LLM 클라이언트 구현 (OpenAI, Anthropic 등)
├── models.py      # 요청/응답 모델 (Pydantic v2)
├── utils.py       # 유틸리티 (API 키 검증, 캐싱)
├── metrics.py     # 성능 메트릭 및 추적
...
from core.llm.clients import OpenAIClient, AnthropicClient
from core.llm.utils import validate_api_key, cache_response
...
### 📊 **데이터 수집 모듈화 (core/ingest/)**

**모듈 구조** (api/ingest.py 분해 완료):
...
from core.ingest.validator import validate_ticket_data
...
- 🚨 **Import 경로 변경**: 기존 `from api.ingest import ingest` → `from core.ingest import ingest`
- 🚨 **함수 위치**: 핵심 `ingest` 함수는 `core/ingest/processor.py`에 위치
- 🚨 **API 분리**: `/ingest` 엔드포인트는 `api/routes/ingest.py`에서 처리
- 🚨 **레거시 지원**: 기존 코드 호환성 유지를 위한 점진적 마이그레이션

---

## 🧠 **LLM 대화 필터링 패턴 (2025년 6월 21일 추가)**

### 💡 **핵심 LLM 대화 필터링 원칙**

**기존 문제점 해결**:

- 5개 대화 제한으로 인한 맥락 손실 → 대화 수 제한 제거
- 다국어 환경 미고려 → 한국어/영어 맞춤 필터링
- 단순한 최신순 선택 → 중요도 기반 스마트 선택

**핵심 구현 원칙**:

- **맥락 보존 우선**: 노이즈 제거는 부차적
- **다국어 지원**: 한국어/영어 각각 다른 필터링 규칙
- **토큰 예산 관리**: LLM 토큰 한계 내 최적 선택

### 🔧 **LLM 대화 필터링 구현 패턴**

#### **1. SmartConversationFilter 클래스 통합**

```python
# backend/core/langchain/llm_manager.py에 통합
...
class LLMManager:
    def __init__(self):
        ...
        self.conversation_filtering_enabled = os.getenv("CONVERSATION_FILTERING_ENABLED", "true").lower() == "true"

    def generate_summary_with_conversations(self, ticket_data: Dict) -> str:
        """대화 포함 요약 생성 - 스마트 필터링 적용"""
        conversations = ticket_data.get('conversations', [])

        if self.conversation_filtering_enabled and conversations:
            # 기존 5개 제한 대신 스마트 필터링 사용
            filtered_conversations = self.smart_filter.filter_conversations_unlimited(conversations)
            logger.info(f"대화 필터링: {len(conversations)}개 → {len(filtered_conversations)}개")
        else:
            # 폴백: 기존 로직 유지
            filtered_conversations = conversations[-5:] if len(conversations) > 5 else conversations
...
def _load_keyword_patterns(self, config_path: str) -> Dict:
    ...
    try:
        ...
        logger.warning(f"키워드 파일 없음: {config_path}, 기본 패턴 사용")
...
def _select_within_token_budget(self, scored_conversations: List[Tuple[Dict, float, str]]) -> List[Dict]:
    ...
    selected = []
    current_tokens = 0
    max_tokens = int(os.getenv("CONVERSATION_TOKEN_BUDGET", "8000"))

    # 필수 대화 유형 우선 선택
    essential_types = ["problem_definition", "solution", "conclusion"]

    for conv, score, conv_type in sorted(scored_conversations, key=lambda x: x[1], reverse=True):
        estimated_tokens = self._estimate_tokens(self._extract_conversation_body(conv))
...
```python
# 필터링 성능 추적 패턴
class ConversationFilteringMetrics:
    def track_filtering_performance(self,
        ...
        # 메트릭 로깅 또는 저장
        logger.info(f"대화 필터링 성능: {metrics}")
```
