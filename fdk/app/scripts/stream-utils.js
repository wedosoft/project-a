...
(function() {
  ...
  /**
   * Submit HITL feedback (helpful/not_helpful)
   * POST /api/assist/feedback/submit
   * @param {string} analysisId - 분석 ID
   * @param {string} eventType - 'helpful' | 'not_helpful'
   * @param {Object} options - { rating?, feedback_text? }
   * @returns {Promise<Object>} { success, event_id }
   */
  async function submitHITLFeedback(analysisId, eventType, options = {}) {
  ...
  /**
   * Submit edited/approved response
   * POST /api/assist/feedback/edit
   * @param {string} analysisId - 분석 ID
   * @param {Object} approvedResponse - 수정된 응답
   * @param {string} agentId - 상담원 ID (optional)
   * @returns {Promise<Object>} { success, sample_id }
   */
  async function submitHITLEdit(analysisId, approvedResponse, agentId = null) {
  ...
  console.log('[StreamUtils] Initialized (with V2 + HITL API)');
